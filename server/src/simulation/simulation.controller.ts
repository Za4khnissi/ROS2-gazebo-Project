...
export class SimulationController {
  ...
  @ApiOperation({
    summary: 'Change drive mode for a specific robot during simulation',
  })
  @ApiOkResponse({ description: 'Drive mode changed successfully' })
  @ApiBadRequestResponse({
    description: 'Bad Request: Invalid Robot ID or drive mode',
  })
  @ApiInternalServerErrorResponse({
    description: 'Internal Server Error: Failed to change drive mode',
  })
  async changeDriveMode(
    ...
    try {
      ...
      return {
        ...
        message: `Drive mode for Robot ${robotId} changed to ${drive_mode}`,
        result,
      };
    } catch (error) {
